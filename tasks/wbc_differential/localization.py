...
class Dataset(DatasetBase):
    def __init__(self,
        ...
        super().__init__()
        self.images = self.__load_images(path / 'color', image_size)
        self.masks = self.__load_images(path / 'mask', target_size)
    ...
    @staticmethod
    def __load_images(path: Path, size: tuple[int, int]) -> list[Tensor]:
        images: list[Tensor] = []
        for entry in path.glob('*/*.png'):
...
class LocalizationMode(Mode):
    ...
    def load_datasets(self, root: Path) -> tuple[Dataset, Dataset]:
        ...
        image_size = (512, 512)
        target_size = (60, 60)
        train_ds = Dataset(root / 'train', image_size,
                           target_size, transform=train_transform)
        val_ds = Dataset(root / 'test', image_size,
                         target_size, transform=None)
